...
export function ButtonAddInteraction({ mutate }: { mutate: () => void }) {
  const { id } = useParams();
  ...
  const handleSave = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    setError("");
    setLoading(true);
    const { ok, data } = await api.post(`/leads/${id}/interactions`, {
      type,
      notes,
    ...
    if (!ok) {
      setError(data.message || "Não foi possível criar a interação.");
    } else {
      setOpen(false);
      mutate();
    }
